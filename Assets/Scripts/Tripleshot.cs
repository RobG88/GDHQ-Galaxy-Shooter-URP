...
public class Tripleshot : MonoBehaviour
{
    ...
    float _autoDestroyThreshold = -10.0f; // Game Screen threshold, once enemy is beyond this point and has been destroyed it will respawn 
    ...
    private void CalculateMovement()
    {
        ...
        if (transform.position.y < _autoDestroyThreshold)
        {
    ...
    */
    void OnTriggerEnter2D(Collider2D other)
    {
        if (other.CompareTag("Player"))
        {
            ...
            if (player != null) {
                Player.instance.TripleshotActivate();
            }
            Destroy(this.gameObject);
        }
    }
